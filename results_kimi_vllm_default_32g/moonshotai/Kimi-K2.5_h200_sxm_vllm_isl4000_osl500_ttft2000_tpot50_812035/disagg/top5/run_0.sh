...
for ((w=0; w<PREFILL_WORKERS; w++)); do
  ...
  CUDA_VISIBLE_DEVICES=$GPU_LIST \
    python3 -m dynamo.vllm \
      --model "$MODEL_PATH" \
\
      --tensor-parallel-size "16" --pipeline-parallel-size "1" --data-parallel-size "1" --kv-cache-dtype "auto" --max-num-seqs "1" --max-num-batched-tokens 5500 --is-prefill-worker --kv-transfer-config '{"kv_connector":"NixlConnector","kv_role":"kv_both"}' 2>&1 | sed "s/^/[Prefill $w] /" ) &
done
