                        ...
                        for moe_ep in moe_ep_list:
                            if dp * tp * pp in num_gpu_list and dp * tp == moe_tp * moe_ep:  # check num gpu and width
                                ...
                                # sglang
                                    ...
                                    # vLLM currently does not support enabling MoE-TP and MoE-EP simultaneously.
                                    # See assertion in `src/aiconfigurator/sdk/operations.py` (MoE dispatch path).
...
def _infer_quantization_fields(raw_config: dict) -> dict[str, object]:
    ...
    # Only a small set of quant algorithms are currently modeled/validated by AIC.
    # Many HF configs (e.g. "compressed-tensors") represent checkpoint storage formats
    # rather than an inference quantization mode we can estimate with our perf tables.
    supported_quant_algos = {"fp8", "fp8_block", "nvfp4", "mxfp4", "float16"}
    if quant_algo is not None and quant_algo not in supported_quant_algos:
        logger.info("Ignoring unsupported quant_algo from HF config: %s", quant_algo)
        quant_algo = None
