...
python3 -m dynamo.frontend --http-port "" 2>&1 | sed "s/^/[Frontend] /" &

AGG_GPU=8
AGG_WORKERS=1
AGG_GPU_OFFSET=0
AGG_SYSTEM_PORT_BASE=${AGG_SYSTEM_PORT_BASE:-${DYN_SYSTEM_PORT1:-8081}}
AGG_EVENT_PORT_BASE=${AGG_EVENT_PORT_BASE:-${DYN_VLLM_KV_EVENT_PORT:-}}
AGG_SIDE_CHANNEL_PORT_BASE=${AGG_SIDE_CHANNEL_PORT_BASE:-${VLLM_NIXL_SIDE_CHANNEL_PORT:-}}
for ((w=0; w<AGG_WORKERS; w++)); do
  ...
  ( DYN_SYSTEM_PORT=$SYSTEM_PORT \
    ...
    --model "$MODEL_PATH" \
\
    --tensor-parallel-size "8" --pipeline-parallel-size "1" --max-model-len "6500" --max-num-seqs "512" --max-num-batched-tokens 6012 2>&1 | sed "s/^/[Worker $w] /" ) &
done
wait
