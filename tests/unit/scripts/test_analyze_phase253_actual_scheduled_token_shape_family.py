...
MODULE_PATH = (
    ...
    / "scripts"
    / "analyze_phase253_actual_scheduled_token_shape_family.py"
)
SPEC = importlib.util.spec_from_file_location(
    "analyze_phase253_actual_scheduled_token_shape_family",
    MODULE_PATH,
)
...
def _write_family(root: Path) -> None:
    ...
    _write_case(
        ...
        65536,
        88.0,
        _tp4_rows(TP4_4K_HOLDOUT, 65536),
        topology_key="tp4_dp2_ep8",
        shape_key="isl4000_osl2000_batch128",
        parallelism={"tp": 4, "dp": 2, "ep": 8},
        isl=4000,
...
def test_shape_family_outputs_six_rows(tmp_path: Path) -> None:
    _write_family(tmp_path)
    out_csv = tmp_path / "phase253.csv"
    out_doc = tmp_path / "phase253.md"

    rows = analyzer.analyze_actual_scheduled_token_shape_family(tmp_path)
    analyzer.write_actual_scheduled_token_shape_family_csv(out_csv, rows)
    analyzer.write_actual_scheduled_token_shape_family_doc(out_doc, rows)
    ...
    assert {row["source"] for row in rows} == {"phase253_actual_scheduled_token_shape_family"}
    ...
    tp8_12k_holdout = rows[5]
    assert tp8_12k_holdout["topology_key"] == "tp8_dp1_ep8"
    assert tp8_12k_holdout["shape_key"] == "isl12000_osl2000_batch128"
    assert tp8_12k_holdout["role"] == "holdout"
    assert tp8_12k_holdout["max_bt"] == "65536"
    assert tp8_12k_holdout["control_max_bt"] == "12000"
    assert tp8_12k_holdout["holdout_max_bt"] == "65536"
    assert tp8_12k_holdout["output_ratio_vs_control"] == "0.957143"
    assert tp8_12k_holdout["rank_sum_p50_scheduled_total_tokens"] == "128.000000"
    assert tp8_12k_holdout["rank_sum_p99_scheduled_total_tokens"] == "12000.000000"
    assert tp8_12k_holdout["rank_sum_max_scheduled_total_tokens"] == "12000"
    assert tp8_12k_holdout["rank_sum_max_fill_ratio"] == "0.183105"
    assert tp8_12k_holdout["rank_sum_p99_fill_ratio"] == "0.183105"
    ...
    assert "Phase253: Actual Scheduled Token Shape Family" in doc
    assert "tp8_dp1_ep8 / 12k2k" in doc
    assert "Default AIC | No-Go" in doc


def test_missing_12k_holdout_fails_fast(tmp_path: Path) -> None:
    _write_family(tmp_path)
    case_dir = tmp_path / TP8_12K_HOLDOUT
    for child in case_dir.iterdir():
        child.unlink()
    case_dir.rmdir()

    with pytest.raises(ValueError, match="missing scenario"):
        analyzer.analyze_actual_scheduled_token_shape_family(tmp_path)


def test_high_budget_must_stay_far_below_configured_budget(tmp_path: Path) -> None:
    _write_family(tmp_path)
    trace_path = tmp_path / TP8_12K_HOLDOUT / "scheduler_trace.jsonl"
    rows = []
...
def test_tp8_duplicate_payload_mismatch_fails_fast(tmp_path: Path) -> None:
    ...
    first["scheduled_context_tokens"] = 11999
    first["scheduled_total_tokens"] = 11999
    first["forward_token_count"] = 11999
...
def test_actual_phase253_csv_can_be_loaded_if_present() -> None:
    csv_path = (
        Path(__file__).resolve().parents[3]
        / "docs"
        / "iter_gap_investigation"
        / "phase253_actual_scheduled_token_shape_family.csv"
    )
    if not csv_path.exists():
        pytest.skip("Phase253 output is generated after analyzer implementation")

    rows = analyzer.read_actual_scheduled_token_shape_family_csv(csv_path)

    assert len(rows) == 6
    assert rows[-1]["scenario"] == TP8_12K_HOLDOUT
