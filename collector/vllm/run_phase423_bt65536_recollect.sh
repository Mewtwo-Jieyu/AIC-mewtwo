...
# Phase423 DP2 bt65536 clean recollect driver.
#
# Measurement-only: prefix caching disabled, independent prompt variants,
# natural KV profiling, iteration details, and metrics polling.
...
SCENARIO="${SCENARIO:-K2.5-tp4ep8dp2-8k2k-bt65536}"
TP="${TP:-4}"
DP="${DP:-2}"
EP="${EP:-8}"
ISL="${ISL:-8000}"
...
snapshot_process_residuals() {
  local path="$1"
  pgrep -af '[v]llm|[r]ay|[A]PIServer|[E]ngineCore|[r]un_openai_fixed_shape_benchmark' >"${path}" || true
}
...
EOF
  ...
  if ! wait_for_service "${serve_log}"; then
    ...
    snapshot_process_residuals "${OUT_ROOT}/process_residual_after.txt"
    return 1
  fi
...
PY
  ...
  snapshot_gpu_apps "${OUT_ROOT}/gpu_compute_apps_after.txt"
  snapshot_process_residuals "${OUT_ROOT}/process_residual_after.txt"
  log "PHASE423_DONE bench_exit=${bench_exit}" | tee -a "${OUT_ROOT}/driver.log"
  return "${bench_exit}"
}
