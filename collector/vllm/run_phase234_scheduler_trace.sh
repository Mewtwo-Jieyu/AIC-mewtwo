...
# Phase234 scheduler trace runner. preflight/source-check are local-only.
# run-one is a real GPU entry and requires PHASE234_ALLOW_GPU_RUN=1.
...
OUT_ROOT="${OUT_ROOT:-docs/iter_gap_investigation/phase234_scheduler_trace}"
...
SERVICE_PID=""
CLEANUP_LOG=""
RUNNER_PATH="${PHASE234_GPU_MODEL_RUNNER_PATH:-}"
RUNNER_BACKUP_PATH=""
...
usage() {
  cat <<'EOF'
Usage:
  run_phase234_scheduler_trace.sh preflight [scenario]
  run_phase234_scheduler_trace.sh source-check
  run_phase234_scheduler_trace.sh run-one <scenario>
  run_phase234_scheduler_trace.sh cleanup
...
AIC_PHASE234_SCHEDULER_TRACE_ROW
scheduled_context_tokens
scheduled_decode_tokens
scheduled_total_tokens
scheduled_context_reqs
...
run_source_check() {
  ...
  contract="$(patch_contract_text)"
  [[ "${contract}" == *"compute_iteration_details(scheduler_output)"* ]] || { echo "missing_compute_iteration_details_contract=true" >&2; exit 1; }
  [[ "${contract}" == *"scheduled_total_tokens"* ]] || { echo "missing_scheduled_total_tokens_contract=true" >&2; exit 1; }
  [[ "${contract}" == *"phase"* ]] || { echo "missing_phase_contract=true" >&2; exit 1; }

  echo "phase234_mode=source-check"
  echo "patch_anchor_compute_iteration_details=true"
  echo "patch_anchor_scheduler_output=true"
  echo "trace_field_scheduled_total_tokens=true"
  echo "trace_field_phase=true"
  echo "trace_schema=$(join_by_comma "${TRACE_SCHEMA[@]}")"
  echo "diagnostic_only=true valid_for_default=false perf_database=false"

  if source_path="$(locate_runner_path 2>/dev/null)"; then
    if [[ -f "${source_path}" ]]; then
      ...
      echo "vllm_source_present=false"
    fi
  else
    echo "vllm_source_present=false"
  fi
...
build_serve_cmd() {
  ...
  SERVE_CMD=(
    env
    AIC_PHASE234_SCHEDULER_TRACE_LOG=1
    "AIC_PHASE234_SCENARIO=${scenario}"
    "AIC_PHASE234_TOPOLOGY_KEY=$(scenario_topology_key "${scenario}")"
    "AIC_PHASE234_SHAPE_KEY=$(scenario_shape_key "${scenario}")"
    "AIC_PHASE234_TP=${tp}"
    "AIC_PHASE234_DP=${dp}"
    "AIC_PHASE234_EP=$(scenario_ep "${scenario}")"
...
print_scenario_contract() {
  ...
  echo "phase234_result_json=$(scenario_dir "${scenario}")/phase234_result.json"
...
run_preflight() {
  ...
  echo "trace_marker=AIC_PHASE234_SCHEDULER_TRACE_ROW"
  echo "trace_schema=$(join_by_comma "${TRACE_SCHEMA[@]}")"
  echo "diagnostic_only=true valid_for_default=false perf_database=false"
...
patch_source() {
  ...
  RUNNER_BACKUP_PATH="${RUNNER_PATH}.phase234.bak"
  [[ -f "${RUNNER_PATH}" ]] || { echo "missing_gpu_model_runner=${RUNNER_PATH}" >&2; exit 1; }
  cp "${RUNNER_PATH}" "${RUNNER_BACKUP_PATH}"
  PHASE234_RUNNER_PATH="${RUNNER_PATH}" python3 - <<'PY'
import os
from pathlib import Path

path = Path(os.environ["PHASE234_RUNNER_PATH"])
...
new = (
    ...
    "                batch_desc.num_reqs if batch_desc.num_reqs is not None else num_reqs\n"
    "            )\n"
    "            if os.environ.get(\"AIC_PHASE234_SCHEDULER_TRACE_LOG\") == \"1\":\n"
    ...
    "                        \"scenario\": os.environ.get(\"AIC_PHASE234_SCENARIO\", \"unknown\"),\n"
    "                        \"iteration\": int(getattr(self, \"_aic_phase234_iter\", 0)),\n"
    "                        \"phase\": aic_phase,\n"
    ...
    "                        \"tp\": int(os.environ.get(\"AIC_PHASE234_TP\", getattr(self.parallel_config, \"tensor_parallel_size\", 1))),\n"
    "                        \"dp\": int(os.environ.get(\"AIC_PHASE234_DP\", getattr(self.parallel_config, \"data_parallel_size\", 1))),\n"
    "                        \"ep\": int(os.environ.get(\"AIC_PHASE234_EP\", \"8\")),\n"
    "                        \"topology_key\": os.environ.get(\"AIC_PHASE234_TOPOLOGY_KEY\", \"unknown\"),\n"
    "                        \"shape_key\": os.environ.get(\"AIC_PHASE234_SHAPE_KEY\", \"unknown\"),\n"
    "                        \"diagnostic_only\": True,\n"
    "                        \"valid_for_default\": False,\n"
    "                        \"perf_database\": False,\n"
    "                    }\n"
    "                    print(\"AIC_PHASE234_SCHEDULER_TRACE_ROW \" + json.dumps(aic_payload, sort_keys=True), flush=True)\n"
    "                self._aic_phase234_iter = int(getattr(self, \"_aic_phase234_iter\", 0)) + 1\n"
    "            ubatch_slices, ubatch_slices_padded = maybe_create_ubatch_slices(\n"
)
...
extract_trace_jsonl() {
  local serve_log="$1"
  local trace_jsonl="$2"
  PHASE234_SERVE_LOG="${serve_log}" PHASE234_TRACE_JSONL="${trace_jsonl}" python3 - <<'PY'
import json
import os
from pathlib import Path

marker = "AIC_PHASE234_SCHEDULER_TRACE_ROW "
serve_log = Path(os.environ["PHASE234_SERVE_LOG"])
trace_jsonl = Path(os.environ["PHASE234_TRACE_JSONL"])
rows = []
for line in serve_log.read_text(encoding="utf-8", errors="replace").splitlines():
    ...
    rows.append(row)
if not rows:
    raise SystemExit("missing_phase234_scheduler_trace_rows=true")
with trace_jsonl.open("w", encoding="utf-8") as f:
    for row in rows:
        f.write(json.dumps(row, sort_keys=True) + "\n")
print(f"scheduler_trace_rows={len(rows)}")
PY
}
...
write_result_json() {
  ...
  PHASE234_SCENARIO="${scenario}" \
  PHASE234_TOPOLOGY_KEY="$(scenario_topology_key "${scenario}")" \
  PHASE234_SHAPE_KEY="$(scenario_shape_key "${scenario}")" \
  PHASE234_TP="$(scenario_tp "${scenario}")" \
  PHASE234_DP="$(scenario_dp "${scenario}")" \
  PHASE234_EP="$(scenario_ep "${scenario}")" \
  PHASE234_ISL="$(scenario_isl "${scenario}")" \
  PHASE234_MAX_BT="$(scenario_max_bt "${scenario}")" \
  PHASE234_MAX_NUM_SEQS="${MAX_NUM_SEQS}" \
  PHASE234_BENCH_NUM_PROMPTS="${BENCH_NUM_PROMPTS}" \
  PHASE234_SERVE_COMMAND="${serve_command}" \
  PHASE234_BENCHMARK_COMMAND="${benchmark_command}" \
  PHASE234_BENCH_JSON="${PHASE234_TEST_BENCH_JSON:-${out_dir}/bench_result.json}" \
  PHASE234_TRACE_JSONL="${PHASE234_TEST_TRACE_JSONL:-${out_dir}/scheduler_trace.jsonl}" \
  PHASE234_OUT_JSON="${PHASE234_TEST_OUT_JSON:-${out_dir}/phase234_result.json}" \
  python3 - <<'PY'
import json
import os
...
bench = json.loads(Path(os.environ["PHASE234_BENCH_JSON"]).read_text(encoding="utf-8"))
trace_rows = [
    json.loads(line)
    for line in Path(os.environ["PHASE234_TRACE_JSONL"]).read_text(encoding="utf-8").splitlines()
    if line.strip()
]
if int(bench.get("failed_requests", -1)) != 0:
    raise SystemExit(f"failed_requests={bench.get('failed_requests')}")
if int(bench.get("ok_requests", 0)) != int(os.environ["PHASE234_BENCH_NUM_PROMPTS"]):
    raise SystemExit("ok_requests does not match requested prompt count")
if not trace_rows:
    raise SystemExit("missing scheduler trace rows")
...
expected_values = {
    "scenario": os.environ["PHASE234_SCENARIO"],
    "topology_key": os.environ["PHASE234_TOPOLOGY_KEY"],
    "shape_key": os.environ["PHASE234_SHAPE_KEY"],
    "max_num_batched_tokens": int(os.environ["PHASE234_MAX_BT"]),
    "max_num_seqs": int(os.environ["PHASE234_MAX_NUM_SEQS"]),
    "tp": int(os.environ["PHASE234_TP"]),
    "dp": int(os.environ["PHASE234_DP"]),
    "ep": int(os.environ["PHASE234_EP"]),
}
...
for row_index, row in enumerate(trace_rows):
    ...
    if missing:
        raise SystemExit(
            f"trace_row_missing_field=row_{row_index}:{','.join(missing)}"
        )
    if row["source"] != "phase234_vllm_scheduler_trace":
    ...
    if row["scheduled_total_tokens"] != (
        row["scheduled_context_tokens"] + row["scheduled_decode_tokens"]
    ):
        raise SystemExit(f"trace_row_token_sum_mismatch=row_{row_index}")
    if row["scheduled_total_reqs"] != (
        row["scheduled_context_reqs"] + row["scheduled_decode_reqs"]
    ):
        raise SystemExit(f"trace_row_request_sum_mismatch=row_{row_index}")

payload = {
    "source": "phase234_scheduler_trace",
    "scenario": os.environ["PHASE234_SCENARIO"],
    "topology_key": os.environ["PHASE234_TOPOLOGY_KEY"],
    "shape_key": os.environ["PHASE234_SHAPE_KEY"],
    "bench_result": {k: v for k, v in bench.items() if k != "records"},
    "shape": {
        "isl": int(os.environ["PHASE234_ISL"]),
    ...
    "runtime": {
        "serve_command": os.environ["PHASE234_SERVE_COMMAND"],
        "benchmark_command": os.environ["PHASE234_BENCHMARK_COMMAND"],
    },
    "diagnostic_only": True,
    "valid_for_default": False,
    "perf_database": False,
}
Path(os.environ["PHASE234_OUT_JSON"]).write_text(
...
run_one() {
  ...
  scenario_index "${scenario}" >/dev/null
  if [[ "${PHASE234_ALLOW_GPU_RUN:-0}" != "1" ]]; then
    echo "phase234_allow_gpu_run_required=true" >&2
    exit 2
  fi
  ...
  trace_jsonl="${out_dir}/scheduler_trace.jsonl"
  : > "${run_log}"
  : > "${CLEANUP_LOG}"
  trap cleanup EXIT
  ...
  echo "phase234_result_json=${out_dir}/phase234_result.json"
}
...
run_test_validate_trace() {
  ...
  [[ -n "${scenario}" ]] || { usage >&2; exit 2; }
  scenario_index "${scenario}" >/dev/null
  [[ -n "${PHASE234_TEST_BENCH_JSON:-}" ]] || { echo "missing_phase234_test_bench_json=true" >&2; exit 2; }
  [[ -n "${PHASE234_TEST_TRACE_JSONL:-}" ]] || { echo "missing_phase234_test_trace_jsonl=true" >&2; exit 2; }
  [[ -n "${PHASE234_TEST_OUT_JSON:-}" ]] || { echo "missing_phase234_test_out_json=true" >&2; exit 2; }
  write_result_json "${scenario}" "$(dirname "${PHASE234_TEST_OUT_JSON}")" "test_serve_command" "test_benchmark_command"
  echo "phase234_trace_guard=PASS"
}
