...
# Phase442 B2: vLLM graph-outer CUDA event timing.
#
# This runner patches the collection node's installed vLLM only for the run,
# emits lightweight JSONL from gpu_model_runner, and restores the file on exit.
# It does not use CUPTI or torch profiler.

WORKDIR="${WORKDIR:-/mnt/shared-storage-user/ailab-sys/zhaojieyu/aic}"
OUT_ROOT="${OUT_ROOT:-docs/iter_gap_investigation/phase442_b2_event_timing}"
TMP_ROOT="${TMP_ROOT:-/tmp/phase442_b2_event_timing_$$}"
PORT="${PORT:-20942}"
FORCE="${FORCE:-0}"
...
PATCH_BACKUP_PATH="${PATCH_BACKUP_PATH:-/tmp/gpu_model_runner.py.phase442.bak}"
PATCHER_REL="${PATCHER_REL:-collector/vllm/phase442_event_timing_patch.py}"
...
run_point() {
  ...
  if [[ "${event_enabled}" == "1" ]]; then
    export AIC_PHASE442_EVENT_JSONL="${label_root}/event_timing.jsonl"
    : >"${AIC_PHASE442_EVENT_JSONL}"
  else
    unset AIC_PHASE442_EVENT_JSONL
  fi

  log "run_point label=${label} event_enabled=${event_enabled} scenario=${scenario}"
  PHASE_NAME="phase442_b2" \
  OUT_ROOT="${label_root}" \
  ...
  if [[ "${event_enabled}" == "1" ]]; then
    local events
    events="$(wc -l <"${AIC_PHASE442_EVENT_JSONL}" | tr -d ' ')"
    if [[ "${events}" == "0" ]]; then
      log "event_jsonl_empty=${AIC_PHASE442_EVENT_JSONL}"
      return 1
    fi
  fi
}
...
payload = {
    "gate": "phase442_event_timing_overhead",
    "metric": "output_tok_s",
...
write_meta() {
  mkdir -p "${OUT_ROOT}"
  python3 - "${OUT_ROOT}/meta.json" "${MODE}" "${SCENARIO}" "${ISL}" "${OSL}" "${MAX_NUM_BATCHED_TOKENS}" "${BENCH_NUM_PROMPTS}" "${BENCH_MAX_CONCURRENCY}" "${MAX_MODEL_LEN}" <<'PY'
import json
import sys

path, mode, scenario, isl, osl, max_bt, prompts, concurrency, max_model_len = sys.argv[1:]
payload = {
    "phase": "phase442_b2_event_timing",
    "mode": mode,
    "scenario": scenario,
    "isl": int(isl),
    "osl": int(osl),
...
main() {
  ...
  case "${MODE}" in
    ...
    event_on)
      patch_apply
      run_point "event_on" "1" "${PORT}" "${SCENARIO}" "${ISL}" "${OSL}" \
        "${MAX_NUM_BATCHED_TOKENS}" "${BENCH_NUM_PROMPTS}" "${BENCH_MAX_CONCURRENCY}" "${MAX_MODEL_LEN}" \
        2>&1 | tee -a "${OUT_ROOT}/driver.log"
      ;;
    calibration_32k)
      patch_apply
      run_point "calibration_32k" "1" "${PORT}" "K2.5-tp4ep8dp2-32k3k" "32000" "3000" \
        "32000" "${BENCH_NUM_PROMPTS}" "${BENCH_MAX_CONCURRENCY}" "262144" \
        2>&1 | tee -a "${OUT_ROOT}/driver.log"
      ;;
    restore_only)
      PATCH_APPLIED=1
