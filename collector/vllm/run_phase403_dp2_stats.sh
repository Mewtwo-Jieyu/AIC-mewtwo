...
# Phase403 DP2 stats recollect driver.
#
# Runs the two prefix-off DP2 clean points from Phase400 and polls /metrics
# during the benchmark to capture running batch, waiting requests, KV usage,
# and preemption counters. This is measurement-only: it does not change
...
OUT_ROOT="${OUT_ROOT:-docs/iter_gap_investigation/phase403_dp2_stats}"
TMP_ROOT="${TMP_ROOT:-/tmp/phase403_dp2_stats_$$}"
PORT_BASE="${PORT_BASE:-20800}"
...
{"name": "${name}", "phase": "phase403", "tp": ${tp}, "dp": ${dp}, "ep": ${ep}, "isl": ${isl}, "osl": ${osl}, "max_num_batched_tokens": ${max_bt}, "batch_size": ${BENCH_MAX_CONCURRENCY}, "world_size": $((tp * dp)), "port": ${port}, "prefix_caching": false, "gpu_memory_utilization": ${GPU_MEMORY_UTILIZATION}, "max_model_len": ${MAX_MODEL_LEN}, "max_num_seqs": ${MAX_NUM_SEQS}, "metrics_poll_interval_s": ${METRICS_POLL_INTERVAL_S}}
EOF
  ...
  start_metrics_poller "${port}" "${metrics_jsonl}"
